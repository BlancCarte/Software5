...
public class GetMenuFavorite {
	...
	public GetMenuStatistic getMenuArr() {
		...
		ResultSet rs = null;
//		SELECT DishNum, SUM(`DishCount`)
//		FROM `order`
//		GROUP BY DishNum
//		order BY SUM(`DishCount`) desc

//		SELECT DishId, sum(DishCount)
//		FROM `order` INNER JOIN `orderlist` ON `order`.orderNum  = `orderlist`.orderNum 
//		WHERE ReserveDate >= "2021-05-01" AND ReserveDate <= "2021-05-31"
//		GROUP BY DishId;

//		SELECT `order`.DishNum, `dish`.DishName , SUM(`order`.DishCount) FROM `order` INNER JOIN `dish` ON `order`.DishNum = `dish`.DishNum "
//				+ "GROUP BY DishNum order BY SUM(`order`.DishCount) desc
